...
public:
	...
	EventJobFinished(const EventJobFinished&);
	EventJobFinished& operator=(const EventJobFinished&);
...
class EventNewJobArrived : public TimedCommand {
	...
	EventNewJobArrived(const EventNewJobArrived&);
	EventNewJobArrived& operator=(const EventNewJobArrived&);
	virtual void Execute();
};
