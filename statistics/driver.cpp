...
//prints content of the bin, since we want enumerate bins - add state (counter)
struct PrintBin {
    int counter;
    PrintBin() : counter(0) {}
...
void test3() {
    ...
    std::cout << "Total number of samples " << std::accumulate( bins.begin(), bins.end(), 0 ) << std::endl;
...
void test4() {
    ...
    std::cout << "Bins:\n";
    std::vector<int> bins = stat1.Histogram( 5, stat1.Minimum(), stat1.Maximum()+1 ); // "+1" - so that 5 is the beginning of the 5th bin
    std::for_each( bins.begin(), bins.end(), PrintBin() );
...
void test7() {
    ...
    std::cout << "Bins:\n";
    std::vector<int> bins = stat3.Histogram( 50, stat3.Minimum(), stat3.Maximum()+1 );
    std::for_each( bins.begin(), bins.end(), PrintBin() );
    std::cout << "Total number of samples " << std::accumulate( bins.begin(), bins.end(), 0 ) << std::endl;
